...
# ------------------ TITLE ------------------
st.markdown("<h1 style='text-align: center;'>📊 Walmart Sales Intelligence Dashboard</h1><hr>", unsafe_allow_html=True)
...
with col2:
    st.subheader("🚨 Alerts")
    if alerts.empty:
        st.success("No major drops")
    else:
        st.dataframe(alerts[['Store','Date','Weekly_Sales']], height=300)
...
st.write(f"🏆 Top Store: Store {top_store} → ${top_value:,.0f}")
st.write(f"📅 Holiday Avg: ${holiday_avg:,.0f} vs ${non_holiday_avg:,.0f}")
st.write(f"📌 Peak Day: {best_day['Date'].date()} → ${best_day['Weekly_Sales']:,.0f}")
